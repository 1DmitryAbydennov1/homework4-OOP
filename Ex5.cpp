...
int main() 
{
    ...
    std::cout << findMaxElement(intVec) << std::endl;

    std::vector<float> floatVec = {1.1f, 3.3f, 2.2f, 5.5f, 4.4f};
    std::cout << findMaxElement(floatVec) << std::endl;

    std::vector<std::string> stringVec = {"apple", "banana", "cherry"};
    std::cout << findMaxElement(stringVec) << std::endl;

    std::vector<std::pair<int, int>> pairVec = {{1, 2}, {3, 4}, {2, 5}, {5, 6}, {4, 1}};
    std::pair<int, int> maxPair = findMaxElement(pairVec);
    std::cout << "("<< maxPair.first << ", " << maxPair.second << ")" << std::endl;
